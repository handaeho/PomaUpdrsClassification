...
class PomaDNNLinearConvertH5toTflite:
    def __init__(self, h5_model_name, h5_model_path):
        print('Start Convert and Save DNN-Linear Combined TF-LITE Model with POMA DataSet')
    ...
    def convert_save_models(self):
        ...
        # save
        try:
            tflite_models_path = '/home/aiteam/daeho/PomaUpdrs/TFLite_models/DNN_Linear_TFLite_models/poma_DNN_Linear_TFLite_Model/'

            model_make_date = str(datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
            model_name = f'poma_dnn_linear_tflite_{model_make_date}.tflite'
