...
class UpdrsDNNLinearInferenceTfLiteModel:
    def __init__(self):
        print('Inference Test for DNN-Linear Combined TF-Lite Model with UPDRS DataSet')
    ...
    def tf_lite_inference(self, model_path, inputs, labels):
        ...
        input_index_dnn = interpreter.get_input_details()[0]["index"]
        input_index_linear = interpreter.get_input_details()[1]["index"]
        ...
        for data in inputs:
            ...
            interpreter.resize_tensor_input(input_index_dnn, [test_data.shape[0], test_data.shape[1]])

            interpreter.allocate_tensors()

            # set DNN input's tensor
            interpreter.set_tensor(input_index_dnn, test_data)

            # set Linear input's tensor
            interpreter.set_tensor(input_index_linear, test_data)
