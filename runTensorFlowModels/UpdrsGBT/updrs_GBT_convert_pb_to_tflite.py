...
class UpdrsGBTConvertPbtoTflite:
    def __init__(self, pb_model_name, pb_model_path):
        print('Start Convert and Save GBT TF-LITE Model with UPDRS DataSet')
    ...
    def convert_save_models(self):
        ...
        # Save the model.
        try:
            tflite_models_path = '/home/aiteam/daeho/PomaUpdrs/TFLite_models/GBT_TFLite_models/updrs_GBT_TFLite_Model/'

            model_make_date = str(datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
            model_name = f'updrs_gbt_tflite_{model_make_date}.tflite'
