...
model_dir = './updrs_dnn_linear_test_210607/'  # 모델을 저장할 디렉토리
model_name = 'updrs_dnn_linear_0001'  # 모델명
...
dnn_linear_estimator = tf.estimator.DNNLinearCombinedClassifier(
    config=estimator.RunConfig(model_dir=model_dir),
    n_classes=3,
    # wide(linear) settings
    linear_feature_columns=[tf.feature_column.numeric_column('x', shape=[95])],
    linear_optimizer=tf.keras.optimizers.Ftrl(),
    # deep settings
    dnn_feature_columns=[tf.feature_column.numeric_column('x', shape=[95])],
    dnn_hidden_units=[1024, 512, 256],
    dnn_optimizer=tf.keras.optimizers.Adam())

tf.estimator.train_and_evaluate(dnn_linear_estimator, train_spec=train_spec, eval_spec=eval_spec)
...
for idx, src_path in enumerate(src_paths): # 순회
    if src_ids[idx] not in des_ids: # 신규 모델이라면
...
#
#      class 0       0.98      0.95      0.96      1765
#      class 1       0.93      0.95      0.94      1583
#      class 2       0.93      0.95      0.94       610
#
#     accuracy                           0.95      3958
#    macro avg       0.94      0.95      0.95      3958
