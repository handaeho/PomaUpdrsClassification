...
def poma_predict_mlp(dataset):
    # load saved model
    clf_from_joblib = joblib.load('../../multi_layer_perceptron_classifier/mlp_poma_2class_210622.pkl')
...
def updrs_predict_mlp(dataset):
    # load saved model
    clf_from_joblib = joblib.load('../../multi_layer_perceptron_classifier/mlp_updrs_2class_210622.pkl')
...
if __name__ == '__main__':
    ...
    result_poma_predict = poma_predict_mlp(poma_x)

    result_updrs_predict = updrs_predict_mlp(updrs_x)

    print('MLP Saved Model POMA Accuracy: {0: .4f}'.format(accuracy_score(poma_y, result_poma_predict)))
    ...
    print('MLP Saved Model UPDRS Accuracy: {0: .4f}'.format(accuracy_score(updrs_y, result_updrs_predict)))

    print('------------------ Test MLP UPDRS ------------------------')
