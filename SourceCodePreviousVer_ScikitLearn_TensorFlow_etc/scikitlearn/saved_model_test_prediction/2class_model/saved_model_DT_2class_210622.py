...
def poma_predict_dt(dataset):
    # load saved model
    clf_from_joblib = joblib.load('../../decision_tree/dt_poma_2class_210622.pkl')
...
def updrs_predict_dt(dataset):
    # load saved model
    clf_from_joblib = joblib.load('../../decision_tree/dt_updrs_2class_210622.pkl')
...
if __name__ == '__main__':
    ...
    result_poma_predict = poma_predict_dt(poma_x)

    result_updrs_predict = updrs_predict_dt(updrs_x)

    print('DT Saved Model POMA Accuracy: {0: .4f}'.format(accuracy_score(poma_y, result_poma_predict)))
    ...
    print('DT Saved Model UPDRS Accuracy: {0: .4f}'.format(accuracy_score(updrs_y, result_updrs_predict)))
