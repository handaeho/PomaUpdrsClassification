...
def poma_predict_rf(result):
    # load saved model
    clf_from_joblib = joblib.load('../../random_forest/rf_poma_3class_210615.pkl')
...
def updrs_predict_rf(result):
    # load saved model
    clf_from_joblib = joblib.load('../../random_forest/rf_updrs_3class_210615.pkl')
...
if __name__ == '__main__':
    ...
    result_poma_predict = poma_predict_rf(poma_x)

    result_updrs_predict = updrs_predict_rf(updrs_x)

    print('RF Saved Model POMA Accuracy: {0: .4f}'.format(accuracy_score(poma_y, result_poma_predict)))

    print('------------------ Test Random Forest POMA  ------------------------')
    ...
    print('RF Saved Model UPDRS Accuracy: {0: .4f}'.format(accuracy_score(updrs_y, result_updrs_predict)))

    print('------------------ Test Random Forest UPDRS  ------------------------')
