...
# 학습 (grid-search 전)
linear_svc_clf.fit(poma_x_train_scaled, poma_y_train)

# 예측 (grid-search 전)
poma_pred_linear_before = linear_svc_clf.predict(poma_x_test_scaled)

print('grid search 전 Linear 예측 정확도 실제 y_test / 예측 pred: ', accuracy_score(poma_y_test, poma_pred_linear_before))

print('------------------ Before test Linear ------------------------')
print(confusion_matrix(poma_y_test, poma_pred_linear_before))
print(classification_report(poma_y_test, poma_pred_linear_before, target_names=['class 0', 'class 1']))
...
# 학습 (grid-search 후)
grid_linear_svc.fit(poma_x_train_scaled, poma_y_train)
...
poma_pred_linear_after = linear_estimator.predict(poma_x_test_scaled)
print('grid-search 후 Linear test 데이터세트 정확도: {0: .4f}'.format(accuracy_score(poma_y_test, poma_pred_linear_after)))

print('------------------ After test Linear ------------------------')
print(confusion_matrix(poma_y_test, poma_pred_linear_after))
print(classification_report(poma_y_test, poma_pred_linear_after, target_names=['class 0', 'class 1']))


# grid search 전 Linear 예측 정확도 실제 y_test / 예측 pred:  0.7766666666666666
# ------------------ Before test Linear ------------------------
# [[104  43]
#  [ 24 129]]
#               precision    recall  f1-score   support
#
#      class 0       0.81      0.71      0.76       147
#      class 1       0.75      0.84      0.79       153
#
#     accuracy                           0.78       300
#    macro avg       0.78      0.78      0.78       300
# weighted avg       0.78      0.78      0.78       300

# Linear GridSearch 최적 파라미터:  {'C': 5.0, 'loss': 'squared_hinge', 'penalty': 'l2'}
# Linear GridSearch 최고 점수:  0.7616666666666666

# grid-search 후 Linear test 데이터세트 정확도:  0.6933
# ------------------ After test Linear ------------------------
# [[ 96  51]
#  [ 41 112]]
#               precision    recall  f1-score   support
#
#      class 0       0.70      0.65      0.68       147
#      class 1       0.69      0.73      0.71       153
#
#     accuracy                           0.69       300
